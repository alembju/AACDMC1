...
function ReviewReserchPaper() {
  ...
  const[show,setShow] = useState(false)
    ...
    const handleUpdate = (e) => {
        e.preventDefault()
        axios.put(`${BASE_URL}/update/`+id, {NCollege,Department,StudyF,Nadvisor,email,phone,PI,PIPhone,PIemail,Title,FocusA,SPeriodB,SPeriodE,ConductS,Rmethods,Study,Design,geographicA,Slocation,Gender,populations,MicroD,Eelectronically,Tools,Tcollection,Budget,Geocoordinate,Groups,Dsharing,Reasons, pdf})
        .then(res => {
            console.log(res);
            navigate('/')
        })
  ...
  const getPdf = async () => {
    const result = await axios.get(`${BASE_URL}/get-Proposal`);
  ...
  const showPdf = (pdf) => {
     window.open(`${BASE_URL}/ReserchProposal/${pdf}`, "_blank", "noreferrer");
    setPdfFile(`${BASE_URL}/ReserchProposal/${pdf}`)
  };
