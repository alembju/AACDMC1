...
function InsertImage() {
 ...
 /*const submitImage = async (e) => {
  ...
  const getImage = async () => {
    const result = await axios.get(`${BASE_URL}/get-image`)
    console.log(result)
    setAllImage(result.data.data)
  };
  ////////////////////////////upload description///////////
  const handleSubmit = (e) =>{
    e.preventDefault();
    axios.post(`${BASE_URL}/IMG-Description`,{Title,Description})
  ...
  const handleSubmite = (e,post) =>{
    ...
    axios.post(`${BASE_URL}/EVENTSUpload`,{EVENTS1,EVENTS2,EVENTS3,EVENTS1Date,EVENTS2Date,EVENTS3Date})
    .then(result =>{console.log(result) 
    window.location.reload()})
    .catch(err => console.log(err))
  }
  ...
  const HandleDelet = (id,EVENTS1) => {
    ...
    axios.delete(`${BASE_URL}/deletEvent/`+id)
    .then(res => {console.log(res)
    window.location.reload()})
  ...
  const BannerDelet = (id,EVENTS1) => {
    if (window.confirm(`Are you sure you want to delete ${EVENTS1}`)) {
    axios.delete(`${BASE_URL}/deletBaner/`+id)
    .then(res => {console.log(res)
    window.location.reload()})
    .then(errr => console.log(errr))
  }
  ...
  ////////////////////////////get events//////////////
  useEffect(() => {
    axios.get(`${BASE_URL}/EventHeadline`)
