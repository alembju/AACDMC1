...
function FacilityInfo() {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL}/Facility-info`)
    .then(res => {
     setdata(res.data)
     setrecords(res.data);
    })
  ...
  const handleSubmite = (e) =>{
    e.preventDefault();
    axios.post(`${BASE_URL}/Add-Facility`,{HRegion,HSubCity,HWoreda,HFName})
...
const [Prenatal,setPrenatal]=useState()
 ...
 useEffect(()=>{
  fetch(`${BASE_URL}/GetePrenatalDeath`)
  .then((data)=>data.json())
...
const [RelapsingFever,setRelapsingFever]=useState()
 ...
 useEffect(()=>{
  fetch(`${BASE_URL}/GetRelapsingFever`)
  .then((data)=>data.json())
...
console.log(RelapsingFever,"RelapsingFever file is sent")
 ...
 const Delet = (id,EVENTS1) => {
  ...
  axios.delete(`${BASE_URL}/deletHF/`+id)
