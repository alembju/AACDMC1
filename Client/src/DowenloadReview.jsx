  ...
  const[show,setShow] = useState(false)
    ...
    useEffect(()=> {
        const fetchData = async() => {
            try {
                const response = await axios.get(`${BASE_URL}/Acces/`+id);
    ...
    const handleUpdate = (e) => {
        e.preventDefault()
        axios.put(`${BASE_URL}/updateDowenload/`+id, {orgname,title,principal,phone,dsharing,availability,coverage,BGDate,LDate,gender,type,raw,cleaned,url,comments,email,Title,pdf})
        .then(res => {
            console.log(res);
            navigate('/')
        })
